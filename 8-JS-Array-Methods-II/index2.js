...
peopleTotalSalary = people.reduce((totalSalary, person) => {
  return Math.ceil(
    (Number(totalSalary) + Number(person.salary)) / people.length
  );
}, 0);
...
peopleOver30 = people.filter((person) => {
...
peopleDepartment = people.reduce((department, person) => {
  department[person.department]
    ? department[person.department]++
    : (department[person.department] = 1);
  return department;
}, {});
